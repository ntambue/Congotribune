...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.adsCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ads-categories.update", [$adsCategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="code">{{ trans('cruds.adsCategory.fields.code') }}</label>
                <textarea class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" name="code" id="code">{{ old('code', $adsCategory->code) }}</textarea>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories_id" id="categories_id" required>
                    @foreach($categories as $id => $categories)
                        <option value="{{ $id }}" {{ ($adsCategory->categories ? $adsCategory->categories->id : old('categories_id')) == $id ? 'selected' : '' }}>{{ $categories }}</option>
                    @endforeach
                </select>
